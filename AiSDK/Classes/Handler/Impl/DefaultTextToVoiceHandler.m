...
@property(nonatomic, assign) float oldVolumn;
@property(nonatomic, assign) float deviceVolumn;
...
- (void)setupAudioSession {
    ...
    if (error) {
        [AiLogger e:@"设置音频会话类别失败: %@", error.localizedDescription];
        return;
    }
    ...
    if (error) {
        [AiLogger e:@"激活音频会话失败: %@", error.localizedDescription];
    } else {
        [AiLogger i:@"音频会话配置成功，支持后台播放"];
    }
}
...
- (void)handle:(NSString *)text play:(BOOL)play
{
    ...
    [AiLogger i:@"请求ID：%@, wid: %@, dir: %@, text: %@", requestId, wid, dir, text];
    self.converting = YES;
    self.myText = text;
    if (text.length <= 0) {
        [self convertComplete:nil code:-1 errorMsg:@"Text is null"];
        return;
    }
    ...
    [[AFlash shared] textToSpeechWithRequestId:requestId wid:wid thirdUuid:nil text:text fileFormat:@"mp3" onSuccess:^(NSString * _Nonnull requestId, NSData * _Nonnull data) {
        ...
        if (strongSelf) {
            ...
            NSData *crcData = [HwBluetoothCenter crcDataFromData:textData total:4];
            NSString *crc = [crcData hexString];
            
            NSString *filePath = [strongSelf saveMP3Data:data toDirectory:dir withFixedFileName:[NSString stringWithFormat:@"%@.mp3", crc]];
            strongSelf.converting = NO;
...
- (BOOL)isHandling:(NSString *)text
{
    ...
    if (self.myText.length == 0) {
        return NO;
    }
    return [self.myText isEqualToString:text];
}
...
- (void)handle:(NSString *)text state:(NSInteger)state language:(NSInteger)language volumn:(NSInteger)volumn crc:(NSString *)crc
{
    [self setSystemVolumnPercent:volumn];
    NSString *dir = [AiFileUtils generateTextToVoicePath];
    
    NSString *mp3Path = [dir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp3", crc]];
    if ([[NSFileManager defaultManager] fileExistsAtPath:mp3Path]) {
        [self convertComplete:mp3Path code:0 errorMsg:nil];
        return;
    }
    
    NSString *txtPath = [dir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.txt", crc]];
    if ([[NSFileManager defaultManager] fileExistsAtPath:txtPath]) {
        NSError *error = nil;
        NSString *content = [NSString stringWithContentsOfFile:txtPath encoding:NSUTF8StringEncoding error:&error];
        if (content) {
            [AiLogger i:@"读取txt成功：%@", content];
            [self handle:content play:YES];
            return;
        }
        [AiLogger e:@"读取失败：%@", error.localizedDescription];
    }
    ...
    if (text.length > 0) {
        ...
        NSString *mp3Path = [dir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp3", hex]];
        if ([[NSFileManager defaultManager] fileExistsAtPath:mp3Path]) {
            [self convertComplete:mp3Path code:0 errorMsg:nil];
            return;
        }
               ...
               errorMsg:(NSString *)errorMsg
{
    if (self.isCanceled) {
        ...
        [self resetSystemVolumn];
        return;
    }
    [[AiSDK sharedInstance] textToVoiceCompleted:text code:code msg:errorMsg];
    if (code != 0) {
...
- (void) setSystemVolumnPercent:(NSInteger)volumn {
    [AiLogger i:@"设置音量: %@", @(volumn)];
    float volumnP = (float) volumn / 100.0f;
    self.oldVolumn = [[VolumeManager sharedManager] getCurrentVolume];
    [[VolumeManager sharedManager] setSystemVolume:volumnP];
    self.deviceVolumn = volumnP;
    [AiLogger i:@"最终音量：%@", @(volumnP)];
}

- (void) resetSystemVolumn {
    if (self.oldVolumn < 0) {
        return;
    }
    
    float current = [[VolumeManager sharedManager] getCurrentVolume];
    // 如果设备音量和系统音量不一致，说明用户修改了系统音量
    if (ABS(self.deviceVolumn - current) > 0.01) {
        return;
    }
    [AiLogger i:@"恢复音量：%@", @(self.oldVolumn)];
    [[VolumeManager sharedManager] setSystemVolume:self.oldVolumn];
}
